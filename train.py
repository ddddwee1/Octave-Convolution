...
class FaceResNet(M.Model):
	...
	def forward(self, x, label):
		feat = self.resnet(x)
		logits = self.classifier(feat, label, 1.0, 0.2, 0.0) # smaller constraint is needed in first epochs. After convergence, change from 0.2 to 0.5
		logits = logits * 64
		return logits
	...
	for ep in range(EPOCH):
		...
		for it in range(data_reader.iter_per_epoch):
			...
			if it%10==0:
				...
				t0 = t1 
				print('Epoch:%d\tIter:%d\tLoss:%.6f\tAcc:%.6f\tSpeed:%.2fIMG/SEC\tLR:%f'%(ep, it, ls, acc, img_sec, LRV))
